...
function Admin() {
  ...
  async function submit(e){
    ...
    try{
      await axios.post("http://localhost:9000/admin",
        {addmision,password}
      )
      .then(res=>{
        // console.log("hii");
        ...
        <MDBCol className='col1' col='12'>
          ...
          <MDBCard className='text-white card topform' style={{borderRadius: '1rem', maxWidth: '400px'}}>
            <MDBCardBody className='p-5 d-flex flex-column align-items-center mx-100 w-100'>
              ...
              <h2 className="fw-bold mb-2 text-uppercase ttext">Admin Login</h2>
              <p className="text-center ttext1 mb-5 text-sm-start">Please Enter Your Credentials!</p>

              <form action='POST'>
                <div className="input-icon">
                  <MDBInput onChange={(e)=>{setAddmision(e.target.value)}} wrapperClass='mt-3 w-100' labelClass='text-white' placeholder="Admin Id" id='formControlLg' type='email' size="lg" icon="user" />
